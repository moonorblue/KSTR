...
#calculate reconstruction route socres
...
#split input routes into pair dictionary
...
#reconstruct routes from split dictionary
...
for p in pro: 
    ...
    for row in rows:
        ...
        # get original route information
        orignalCategory = Set([])
        orignalPOI = Set([])
        POIs = eval(row[0])
        for POI in POIs:
        ...
        #split routes into pair dictionary
        ...
        #use pair dictionary to reconstruct routes 
        for i in splitDict:
            tempStack = []
            construct_c = 0
        ...
        with open('/home/moonorblue/exp/topk_6/ca/expV6_ca_parallel_skyline_total_user_checkincount_'+str(p)+'.csv', 'a') as w:
            for d in resultData:
            ...
            w.close()
        with open('/home/moonorblue/exp/topk_6/ca/expV6_ca_parallel_skyline_avg_user_checkincount_'+str(p)+'.csv', 'a') as w:
            ...
            w.close()
        with open('/home/moonorblue/exp/topk_6/ca/expV6_ca_parallel_skyline_user_checkincount_time_'+str(p)+'.csv', 'a') as y:

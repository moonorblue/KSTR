...
def splitIntoHeadDict(route):
    ...
    for i in xrange(length):
        ...
        if head[0] not in splitDict:
            ...
            splitDict[head[0]] = {}
            splitDict[head[0]][tail[0]] = (head,tail),tailScore
        else:
            if tail[0] in splitDict[head[0]]:
                if tailScore > splitDict[head[0]][tail[0]][1]:
                    splitDict[head[0]][tail[0]] = (head,tail),tailScore
                
            # splitDict[head[0]].add((head,tail))


def construct(pairDict):
    endflag = False
    # print pairDict
    global construct_c 
    ...
    for i in pairDict:
        if len(tempStack) == 0:
            tempStack.append(pairDict[i][0][0])
            tempStack.append(pairDict[i][0][1])
            # tempStack.append(i[0])
            # tempStack.append(i[1])
        else:
            idx = len(tempStack) - 1
            if pairDict[i][0][1][1] > tempStack[idx][1] and pairDict[i][0][1][0] != tempStack[idx][0]:
                 tempStack.append(pairDict[i][0][0])
                 tempStack.append(pairDict[i][0][1])
            else:
                 endflag = True
        
        headSet = splitDict.get(pairDict[i][0][0][0],None)
        tailSet = splitDict.get(pairDict[i][0][1][0],None)
...
conn_string = "host='192.168.100.200' dbname='moonorblue' user='moonorblue' password='4321'"
conn = pg.connect(conn_string)
query = "SELECT a.poi,a.region_minlong,a.region_minlat,a.region_maxlong,a.region_maxlat,a.uid,a.rid,a.geom FROM ca_route a,ca_checkincount b WHERE b.uid = a.uid AND st_area(a.geom) != 0 ORDER BY b.checkincount,a.rid LIMIT 1 ;"
...
for p in pro: 
    ...
    log = open('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_total_user_checkincount_log_'+str(p),'a')
    ...
    for row in rows:
        ...
        for i in splitDict:
            ...
            # for k in splitDict[i]:
                # construct(splitDict[i][k][0]) 
        ...
        with open('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_total_user_checkincount_'+str(p)+'.csv', 'a') as w:
            for d in resultData:
            ...
            w.close()
        with open('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_avg_user_checkincount_'+str(p)+'.csv', 'a') as w:
            for d in resultData_avg:
                w.write(str(d[0]) + ',' + str(d[1]) + ',' + str(d[2]) +
                        ',' + str(d[3]) + ',' + str(d[4]) + ',' + str(d[5]) +','+str(d[6])+','+str(d[7])+','+str(d[8])+','+str(d[9])+','+str(d[18])+','+str(d[15])+','+str(d[16])+','+str(d[17])+ '\n')

            w.close()
        with open('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_user_checkincount_time_'+str(p)+'.csv', 'a') as y:
        ...
        if progressCount in StopNumbers:
            shutil.copyfile('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_total_user_checkincount_'+str(p)+'.csv', '/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_total_user_checkincount_'+str(p)+'_'+str(progressCount)+'.csv')
            shutil.copyfile('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_avg_user_checkincount_'+str(p)+'.csv', '/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_avg_user_checkincount_'+str(p)+'_'+str(progressCount)+'.csv')
            shutil.copyfile('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_user_checkincount_time_'+str(p)+'.csv', '/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_user_checkincount_time_'+str(p)+'_'+str(progressCount)+'.csv')
        # with open('/home/moonorblue/exp/topk_2/ca/expV6_ca_parallel_skyline_avg_user_checkincount_time_'+str(p)+'.csv', 'a') as y:

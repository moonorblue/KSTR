...
query = "SELECT a.poi,a.region_minlong,a.region_minlat,a.region_maxlong,a.region_maxlat,a.uid,a.rid,a.geom  FROM ca_route a WHERE a.rid = '95711' ;" 
    ...
    for row in rows:
        ...
        for testtime in xrange(100):   
            ...
            for processc in processList:
                ...
                # qByRegion = "SELECT poi,rid,(st_area(st_intersection(geom,'" + str(row[7])+ "'))/st_area('"+str(row[7])+"'))*100 FROM ca_route WHERE geom && '"+str(row[7])+"' AND st_area(geom) != 0 AND st_area('"+str(row[7])+"') != 0 AND (st_area(st_intersection(geom, '"+str(row[7])+"'))/st_area('"+str(row[7])+"')) != 0 AND rid != "+orignal_rid+"ORDER BY rid LIMIT "+str(rc)+" ;"
                qByRegion = "SELECT poi,rid,(st_area(st_intersection(geom,'" + str(row[7])+ "'))/st_area('"+str(row[7])+"'))*100 FROM ca_route WHERE geom && '"+str(row[7])+"' AND st_area(geom) != 0 AND st_area('"+str(row[7])+"') != 0 AND (st_area(st_intersection(geom, '"+str(row[7])+"'))/st_area('"+str(row[7])+"')) != 0 AND rid != "+orignal_rid+"ORDER BY rid LIMIT 10000 ;"
                ...
                # print len(skylineInputValue_avg)
                ...
                # pool.join()
                # resultData_avg = [result[skylineInputDict_avg[i]] for i in r if i is not None]
                # # skyline_avg2 = timer.time()-skyline_start_avg  
                # # print skyline_avg,skyline_avg2         

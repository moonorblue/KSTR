...
# query = "SELECT a.poi,a.region_minlong,a.region_minlat,a.region_maxlong,a.region_maxlat,a.uid,a.rid,a.geom FROM fb_route a,fb_checkincount b WHERE b.uid = a.uid AND st_area(a.geom) != 0 AND a.rid ='104878' ORDER BY b.checkincount,a.rid  ;"
query = "SELECT a.poi,a.region_minlong,a.region_minlat,a.region_maxlong,a.region_maxlat,a.uid,a.rid,a.geom  FROM fb_route a WHERE a.rid = '137' ;" 
...
for p in pro: 
    ...
    length = len(rows)
    # routescount = [100,200,400,800,1500,3000,5000,8000,12000,15000,18000,21000,24000,27000,30000] 
    processList = [1,2,4,8,16,32]
    # timeDict = {i:0 for i in routescount }
    # timeDict2 = {i:0 for i in routescount }
    proDict = {i:0 for i in processList}
    progressCount = 1

    for row in rows:

        for testtime in xrange(100):     
            print testtime       
            # for rc in routescount:
            for processc in processList:
                orignal_rid = str(row[6])
                # qByRegion = "SELECT poi,rid,(st_area(st_intersection(geom,'" + str(row[7])+ "'))/st_area('"+str(row[7])+"'))*100 FROM fb_route WHERE geom && '"+str(row[7])+"' AND st_area(geom) != 0 AND st_area('"+str(row[7])+"') != 0 AND (st_area(st_intersection(geom, '"+str(row[7])+"'))/st_area('"+str(row[7])+"')) != 0 AND rid != "+orignal_rid+" ORDER BY rid LIMIT "+str(rc)+" ;"
                qByRegion = "SELECT poi,rid,(st_area(st_intersection(geom,'" + str(row[7])+ "'))/st_area('"+str(row[7])+"'))*100 FROM fb_route WHERE geom && '"+str(row[7])+"' AND st_area(geom) != 0 AND st_area('"+str(row[7])+"') != 0 AND (st_area(st_intersection(geom, '"+str(row[7])+"'))/st_area('"+str(row[7])+"')) != 0 AND rid != "+orignal_rid+" ORDER BY rid LIMIT 10000 ;"
                ...
                pool_size = processc  # your "parallelness"
                ...
                proDict[processc] += totalTime
                # print totalTime
                # timeDict[rc] += totalTime
                progressCount += 1
    for i in proDict:

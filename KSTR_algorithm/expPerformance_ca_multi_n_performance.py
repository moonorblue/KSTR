...
rowDict = json.load(open('/home/moonorblue/exp/materials/CA_coverrows_100'))
timeDict = {}
rw=1
for rowCount in rowDict:
    ...
    for tester in testData:
        ...
        query = "SELECT a.poi,a.region_minlong,a.region_minlat,a.region_maxlong,a.region_maxlat,a.uid,a.rid,a.geom  FROM ca_route a WHERE a.rid = "+str(tester)+" ;" 
        ...
        for p in pro: 
            ...
            for row in rows:
                ...
                # for rc in routescount:
                    ...
                    qByRegion = "SELECT poi,rid,(st_area(st_intersection(geom,'" + str(row[7])+ "'))/st_area('"+str(row[7])+"'))*100 FROM ca_route WHERE geom && '"+str(row[7])+"' AND st_area(geom) != 0 AND st_area('"+str(row[7])+"') != 0 AND (st_area(st_intersection(geom, '"+str(row[7])+"'))/st_area('"+str(row[7])+"')) != 0 AND rid != "+orignal_rid+"ORDER BY rid ;"
    ...
    rw += 1       
        # print 'Finish!'
print 'Finish'
w = open('/home/moonorblue/exp/n_performance/ca','w')
